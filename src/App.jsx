...
const T = {
  tr: {
    ...
    navPricing: 'Planlar',
    navLogin: 'Giriş Yap',
    navWaitlist: 'Bekleme Listesine Katıl',
    ...
    heroCta: 'Bekleme Listesine Katıl',
    heroFeatures: 'Tüm Özellikler',
    releaseLabel: 'Tahmini Çıkış:',
    releaseSuffix: 'Windows 10 / 11',
    demoSuggestions: 'Öneriler',
    stat1n: '100+', stat1l: 'Araç & Entegrasyon',
    stat2n: '6',    stat2l: 'Desteklenen AI Modeli',
    stat3n: '<200ms',stat3l: 'Hedef Yanıt Süresi',
    stat4n: '%100', stat4l: 'Yerel Çalışma Seçeneği',
    ...
    tBlock1P: "ARIA arka planda ne yaptığını gizlemez. Action Receipt ile hangi modele istek attığını, hangi dosyayı okuduğunu ve kaç token harcadığını anlık olarak izleyin.",
    ...
    tBlock2P: 'Günün sonunda görev listesini bırakın, sabah hazır raporlarla karşılayın. Night Shift 2.0 öz-doğrulama ve kalite kontrol ile otonom görev zincirlerini yönetir.',
    ...
    pricingSub: 'Fiyatlar 2026 Q4 lansmanında açıklanacak. Erken erişim için bekleme listesine kayıt olun.',
    pricingTiers: [
      { name:'Free', price:'Ücretsiz', period:'Çıkışta mevcut', desc:'Bireysel kullanım için temel AI işletim sistemi deneyimi.', feats:['Command OS (Alt+Space)','Günlük 5 web araması','3 Conversation Thread','Temel Clipboard Algılama','Topluluk Desteği'], cta:'Bekleme Listesine Katıl' },
      { name:'Pro',  price:'Yakında',  period:'Fiyat açıklanacak · 2026 Q4', desc:'Night Shift, tam model özgürlüğü, sınırsız bellek ve Multi-Agent erişimi.', feats:['Night Shift 2.0','Sınırsız Model Özgürlüğü (BYOK)','Knowledge Base & RAG','Action Receipt tam loglama','Workflow Builder','Multi-Agent Swarm','Öncelikli Destek'], cta:'Erken Erişim Kaydı', featured:true },
      { name:'Team', price:'Yakında',  period:'Kurumsal fiyatlandırma · 2026 Q4', desc:'Ekipler için ortak hafıza, yönetici paneli ve kurumsal güvenlik.', feats:['Tüm Pro özellikleri','Paylaşılan Knowledge Base','Ekip Workflow\'ları','Admin Denetim Paneli','Cross-device Sync','SSO & Kurumsal güvenlik','Özel SLA'], cta:'Bilgi Al' },
    ],
    ctaBadge: '2026 Q4 Lansmanı',
    ctaH: 'Hazır olun.\nARIA geliyor.',
    ctaP: 'Lansman öncesi bildirim almak ve erken erişim fırsatından yararlanmak için bekleme listesine katılın.',
    ctaBtn: 'Bekleme Listesine Katıl',
    footerDesc: 'Windows için kişisel AI işletim sistemi. 2026 Q4 lansmanı için bekleme listesine katılın.',
    footerProduct: 'Ürün',
  ...
  en: {
    ...
    stat1n: '100+', stat1l: 'Tools & Integrations',
    stat2n: '6',    stat2l: 'Supported AI Models',
    stat3n: '<200ms',stat3l: 'Target Response Time',
    stat4n: '100%', stat4l: 'Local-only Option',
    ...
    pricingSub: 'Pricing will be announced at Q4 2026 launch. Sign up for early access.',
    pricingTiers: [
      { name:'Free', price:'Free',    period:'Available at launch', desc:'The core AI OS experience for individual users.', feats:['Command OS (Alt+Space)','5 web searches/day','3 Conversation Threads','Basic Clipboard Detection','Community Support'], cta:'Join Waitlist' },
      { name:'Pro',  price:'Soon',    period:'Pricing TBA · Q4 2026', desc:'Night Shift, full model freedom, unlimited memory, Multi-Agent access.', feats:['Night Shift 2.0','Unlimited Model Freedom (BYOK)','Knowledge Base & RAG','Full Action Receipt logging','Workflow Builder','Multi-Agent Swarm','Priority Support'], cta:'Early Access Signup', featured:true },
      { name:'Team', price:'Soon',    period:'Enterprise pricing · Q4 2026', desc:'Shared memory, admin panel, and enterprise-grade security for teams.', feats:['All Pro features','Shared Knowledge Base','Team Workflows','Admin Dashboard','Cross-device Sync','SSO & Enterprise Security','Custom SLA'], cta:'Contact Sales' },
    ],
    ctaBadge: 'Q4 2026 Launch',
    ctaH: 'Get ready.\nARIA is coming.',
    ctaP: 'Join the waitlist to get notified before launch and secure your early-access spot.',
    ctaBtn: 'Join the Waitlist',
    footerDesc: 'A personal AI operating system for Windows. Join the waitlist for the Q4 2026 launch.',
    ...
    <nav className="navbar" style={scrolled ? {borderBottomColor:'rgba(255,255,255,0.12)'} : {}}>
      <div className="wrap nav-inner">
        ...
        <div className="nav-cta-group">
          <button
            className="lang-toggle btn btn-ghost"
          ...
          <a href="#waitlist" className="btn btn-solid">
            <BellRing size={13} />{t.navWaitlist}
          </a>
        </div>
      </div>
    </nav>
...
function Hero() {
  ...
  const demoItems = [
    ...
    { Icon: Clipboard,     title: lang==='tr'?'Pano içeriğini analiz et'    :'Analyze clipboard content', sub: lang==='tr'?'Kopyalanan metni veya kodu anında işle'            :'Instantly process copied text or code',           badge:'Clipboard',   color:'#111111' },
    ...
    <section className="hero">
      ...
      <div className="wrap" style={{position:'relative',zIndex:1}}>
        ...
        <div className="hero-actions">
          <a id="waitlist" href="#waitlist-cta" className="btn btn-solid"><BellRing size={15} />{t.heroCta}</a>
...
function FeatureScroll() {
  const { t, lang } = useLang();
  const chips = [
    { Icon: Terminal,      text: lang==='tr'?'Alt+Space Command OS':'Alt+Space Command OS' },
    { Icon: Moon,          text: 'Night Shift 2.0' },
    { Icon: UserCog,       text: lang==='tr'?'Otonom Profil Öğrenimi':'Autonomous Profile Learning' },
    { Icon: Clipboard,     text: lang==='tr'?'Clipboard Intelligence':'Clipboard Intelligence' },
    { Icon: Shuffle,       text: lang==='tr'?'Model Özgürlüğü (BYOK)':'Model Freedom (BYOK)' },
    { Icon: Database,      text: lang==='tr'?'Knowledge Base & RAG':'Knowledge Base & RAG' },
    { Icon: Network,       text: lang==='tr'?'Multi-Agent Swarm':'Multi-Agent Swarm' },
    { Icon: FileText,      text: lang==='tr'?'Action Receipt':'Action Receipt' },
    { Icon: ShieldCheck,   text: lang==='tr'?'Gizlilik & Güvenlik':'Privacy & Security' },
    { Icon: MessageSquare, text: lang==='tr'?'Conversation Threads':'Conversation Threads' },
    { Icon: Workflow,      text: lang==='tr'?'Workflow Builder':'Workflow Builder' },
    { Icon: Search,        text: lang==='tr'?'Web Search & Scraping':'Web Search & Scraping' },
    { Icon: FolderOpen,    text: lang==='tr'?'Dosya Sürükle-Bırak':'Drag & Drop Files' },
    { Icon: GitBranch,     text: lang==='tr'?'GitHub & Gmail Entegrasyonu':'GitHub & Gmail Integration' },
...
function ActionReceiptVisual() {
  const { t } = useLang();
  const rows = [
    { Icon:Cpu,         label: t.lang==='tr'?'Model Seçimi':'Model Selection', val:'claude-3.5-sonnet', success:true,  active:true  },
    { Icon:FolderOpen,  label: t.lang==='tr'?'Okunan Dosya':'File Read',       val:'core/router.py (1.2kb)',            active:false },
    { Icon:Globe,       label: t.lang==='tr'?'Web İsteği':'Web Request',       val:'api.github.com → 200',             active:false },
    { Icon:FileText,    label: t.lang==='tr'?'Üretilen Çıktı':'Output',        val:'847 tokens · 1.2s',                active:false },
    { Icon:CheckCircle, label: t.lang==='tr'?'Panoya Kopyalandı':'Copied',     val: t.lang==='tr'?'Tamamlandı':'Done',  success:true, active:false },
  ];
  return (
    ...
    <div className="feature-visual-card">
      ...
      <div className="visual-body">
        <div className="ns-queue">
          {t.nsTasks.map((name,i) => (
            <div className="ns-task" key={i}><span className={`ns-status ${statuses[i]}`} /><span className="ns-task-name">{name}</span><span className="ns-task-time">{times[i]}</span></div>
          ))}
        </div>
        ...
        <div className="model-grid">
          {t.models.map((m,i) => (
            <div key={i} className={`model-card${sel===i?' selected':''}`} onClick={() => setSel(i)}>
              ...
              <div className={`model-badge ${badges[i]}`}>{m.badge}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="feature-visual-card">
      ...
      <div className="visual-body">
        ...
        <div className="clip-popup">
          ...
          {t.clipActions.map((a,i) => { const IC=icons[i]; return (
            <div key={i} className={`clip-action${active===i?' active':''}`} onMouseEnter={() => setActive(i)}>
              <IC size={13} style={{color:'rgba(255,255,255,0.5)',flexShrink:0}} />
              <span className="clip-action-text">{a}</span>
              <span className="clip-action-kbd">{kbds[i]}</span>
            </div>
          ); })}
        </div>
      </div>
    ...
    <div className="feature-visual-card">
      ...
      <div className="visual-body">
        ...
        <div style={{marginTop:20,padding:'12px 14px',background:'rgba(255,255,255,0.03)',borderRadius:8,border:'1px solid var(--border)'}}>
          ...
          <div style={{fontSize:12,color:'#ccc'}}>{t.swarmTask.split('→').map((p,i) => i===0?<span key={i}>{p}→</span>:<span key={i} style={{color:'#34d399'}}>{p}</span>)}</div>
        </div>
      </div>
    </div>
  );
...
const TESTIMONIALS_TR = [
  { quote: 'Night Shift özelliği olmasa inanmam zor olurdu. Gece saat 02\'de kapatıp yatıyorum, sabah uyandığımda rakip analizi hazır. Bu kadar basit anlatayım.', name: 'Burak A.', role: 'Backend Developer · İstanbul', handle: '@burak_dev' },
  { quote: 'Clipboard Intelligence\'ı ilk gördüğümde "bu çok basit bir şey" dedim. Bir haftadır kullanıyorum, terminaldeki her hata logunu kopyaladığımda ARIA zaten ekranımda. Refleks oldu.', name: 'Zeynep Kaya', role: 'Data Scientist · Ankara', handle: '@zeynepdatası' },
  { quote: 'BYOK desteği oyun değiştirici. Anthropic anahtarımı ekledim, şirkete ait verileri yerel Ollama\'ya veriyorum. Güvenlik ekibi de mutlu, ben de.', name: 'Mert D.', role: 'CTO · SaaS Startup, İzmir', handle: '@mertd_cto' },
  { quote: 'Action Receipt olmasaydı bu kadar güvenmezdim. Her şeyin kayıtlı olduğunu görmek, hangi API\'ya istek gittiğini bilmek — bu şeffaflık başka hiçbir araçta yok.', name: 'Selin Arslan', role: 'AI Araştırmacısı · ODTÜ', handle: '@selinai' },
  { quote: 'Multi-agent swarm\'ı test ettim: "şu proje için teknik döküman hazırla" dedim. Router ajan konuyu 4 parçaya böldü, writer ve researcher birlikte çalıştı. 18 dakikada 12 sayfalık döküman.', name: 'Emre Yılmaz', role: 'Lead Engineer · Fintech', handle: '@emre_eng' },
  { quote: 'Türkçe komutlara verdiği yanıtların kalitesi beni şaşırttı. Çoğu AI aracında Türkçe "yeterince iyi" olur. ARIA\'da ana dil gibi.', name: 'Ayşe T.', role: 'Ürün Müdürü · E-ticaret', handle: '@ayseproduct' },
];

const TESTIMONIALS_EN = [
  { quote: "Night Shift is the only reason I leave my laptop on overnight now. Queued 5 research tasks before bed, woke up to finished reports. My mornings are completely different.", name: 'Marcus Weber', role: 'Senior Software Engineer · Berlin', handle: '@marcusw_dev' },
  { quote: "The model switching is seamless. I use Private mode for client code, Fast mode for quick lookups, and Best mode when I need Claude-level reasoning. All from the same shortcut.", name: 'Sarah Chen', role: 'Product Manager · San Francisco', handle: '@sarahchen_pm' },
  { quote: "I was skeptical about the clipboard detection. Two weeks in — I genuinely miss it when I'm using another machine. It just becomes part of how you code.", name: 'Ryan O\'Brien', role: 'Full-Stack Developer · Dublin', handle: '@ryanob_code' },
  { quote: "The Action Receipt transparency feature alone sets this apart from every other AI tool I've used. I know exactly what's happening under the hood at all times.", name: 'Alex Rodriguez', role: 'DevOps Engineer · Madrid', handle: '@alexr_devops' },
  { quote: "We tested this in our team — shared Knowledge Base is the real differentiator. Everyone uses the same context. No more re-explaining the codebase to every AI session.", name: 'Lena Müller', role: 'Engineering Lead · Munich', handle: '@lenam_tech' },
  { quote: "Ran a multi-agent research task on a competitor landscape. Router split it into 6 subtasks, agents ran in parallel. Got back a structured report I'd normally spend a full day writing.", name: 'James Park', role: 'AI Researcher · Singapore', handle: '@jamespark_ai' },
];
              ...
              <div className="testimonial-card fade-up" key={i} style={{transitionDelay:(i%3)*0.1+'s'}}>
                ...
                <div className="t-author">
                  ...
                  <div>
                    ...
                    <div className="t-handle">{item.handle}</div>
                  </div>
                </div>
              </div>
            );
...
function WorkflowSection() {
  const { t } = useLang();
  const stepIcons = [Server, SlidersHorizontal, Terminal, Zap];
  return (
    <section className="workflow">
      <div className="wrap">
        ...
        <div className="workflow-steps">
          {t.wfSteps.map((s, i) => { const IC=stepIcons[i]; return (
          ...
          <div className="privacy-card">
            ...
            {t.memoryItems.map((m,i) => { const IC=memIcons[i]; return (
              <div className="memory-item" key={i}>
                <IC size={16} color="rgba(255,255,255,0.4)" className="memory-icon" />
        ...
        <div className="pricing-grid">
          {t.pricingTiers.map((tier, i) => (
            <div key={i} className={`pricing-card${tier.featured?' featured':''} fade-up`} style={{transitionDelay:i*0.1+'s'}}>
              ...
              <div className="tier-price" style={tier.price==='Yakında'||tier.price==='Soon'?{fontSize:28,paddingTop:10}:{}}>{tier.price}</div>
              <div className="tier-period">{tier.period}</div>
              <div className="tier-desc">{tier.desc}</div>
              <ul className="tier-feats">{tier.feats.map((f,j) => <li key={j}>{f}</li>)}</ul>
              <a href="#waitlist-cta" className={`tier-btn ${tier.featured?'tier-btn-solid':'tier-btn-ghost'}`}>{tier.cta}</a>
            </div>
          ))}
        </div>
      </div>
...
// ── Waitlist CTA (no form) ────────────────────────────────────────────────────
function WaitlistCTA() {
  const { t } = useLang();
  return (
    <section className="cta-banner" id="waitlist-cta">
      <div className="wrap">
        <div className="fade-up">
          ...
          <h2>{t.ctaH.split('\n').map((l,i)=><span key={i}>{l}<br/></span>)}</h2>
          <p>{t.ctaP}</p>
          <a href="https://t.me/ariaai_waitlist" target="_blank" rel="noopener noreferrer" className="btn btn-solid" style={{fontSize:16,padding:'14px 32px',display:'inline-flex',gap:8}}>
            <BellRing size={16} />{t.ctaBtn}
          </a>
        </div>
      </div>
    </section>
